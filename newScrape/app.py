...
def login(driver):
    try:
        ...
        driver.find_element(By.ID, 'ycid-input').send_keys(parameters.linkedin_username)
        time.sleep(3)
        driver.find_element(By.ID, 'password-input').send_keys(parameters.linkedin_password)
        time.sleep(1.5)
...
def main(executable_path):
    try:
        driver = setup_driver(executable_path)
        if driver is None:
            print("Failed to set up the driver.")
            return
        ...
        try:
            navigate_to_filter_page(driver, parameters.filter_params)
...
if __name__ == "__main__":
    main(parameters.executable_path)